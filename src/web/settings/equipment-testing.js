...
function buildCommissioningSteps(status) {
    ...
    const auxSafetyStep = {
        id: 'aux-safety',
        title: 'Дополнительные safety-каналы',
        blocking: false,
        tone: 'success',
        label: 'Готово',
        description: 'ADS1115 отвечает за давление куба и резервные safety-каналы, а GPIO1/GPIO3 оставлены под будущие датчики по месту.',
        actions: [
            { type: 'open', label: 'Открыть статусы', section: 'parameters', cardId: 'hardware-status' }
        ]
    };
    if (safetyTriggered.length > 0) {
        ...
        auxSafetyStep.label = 'Проверь ADS';
        auxSafetyStep.description = 'Каналы A2/A3 заведены в инженерные статусы, но сам модуль ADS1115 сейчас не виден. GPIO1/GPIO3 пока остаются резервом под будущие датчики.';
    } else if (safetyReady.length < 2) {
        auxSafetyStep.tone = 'warning';
        auxSafetyStep.label = `${safetyReady.length}/2`;
        auxSafetyStep.description = 'Часть дополнительных каналов уже готова, но live-сигналы на A2/A3 ещё не подтверждены. Перед запуском проверьте питание ADS, входы ADC и конфигурацию.';
    } else if (safetyReserved.length > 0) {
        auxSafetyStep.tone = 'warning';
        auxSafetyStep.label = 'Резерв';
        auxSafetyStep.description = 'Резервные входы и статусы на будущее уже заведены через A2/A3, а прямые каналы под газ и пар оставлены на GPIO1/GPIO3 для отдельной реализации.';
    }
    ...
    return `
        <div class="equipment-module-card">
            <div class="equipment-module-card-head">
                <strong>${channel.label || 'Канал'}</strong>
