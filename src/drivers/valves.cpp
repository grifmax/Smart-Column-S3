...
static bool valveBody = false;
static bool valveTails = false;
...
static ValvePulseState g_valvePulses[5];
...
static bool isDiscreteValvePinAvailable(int pin) {
    return pin >= 0;
}
...
static void applyBodyState(bool open) {
    valveBody = open;
    if (isDemoHardwareSuppressed()) {
        return;
    }
    if (!isDiscreteValvePinAvailable(PIN_VALVE_BODY)) {
        valveBody = false;
        return;
    }
    digitalWrite(PIN_VALVE_BODY, open ? HIGH : LOW);
}

static void applyTailsState(bool open) {
    valveTails = open;
    if (isDemoHardwareSuppressed()) {
        return;
    }
    if (!isDiscreteValvePinAvailable(PIN_VALVE_TAILS)) {
        valveTails = false;
        return;
    }
    digitalWrite(PIN_VALVE_TAILS, open ? HIGH : LOW);
}
...
static void setValveState(Valves::ValveId valve, bool open, bool cancelPulse) {
    ...
    case Valves::ValveId::HEADS:
        ...
        break;
    case Valves::ValveId::BODY:
        applyBodyState(open);
        LOG_D("Valves: Body %s", open ? "OPEN" : "CLOSED");
        break;
    case Valves::ValveId::TAILS:
        applyTailsState(open);
        LOG_D("Valves: Tails %s", open ? "OPEN" : "CLOSED");
        break;
...
void init() {
    ...
    if (isDiscreteValvePinAvailable(PIN_VALVE_BODY)) {
        pinMode(PIN_VALVE_BODY, OUTPUT);
    }
    if (isDiscreteValvePinAvailable(PIN_VALVE_TAILS)) {
        pinMode(PIN_VALVE_TAILS, OUTPUT);
    }
    ...
    if (isDiscreteValvePinAvailable(PIN_VALVE_BODY)) {
        digitalWrite(PIN_VALVE_BODY, LOW);
    }
    if (isDiscreteValvePinAvailable(PIN_VALVE_TAILS)) {
        digitalWrite(PIN_VALVE_TAILS, LOW);
    }
...
void configureFractionator(const FractionatorSettings& settings) {
    for (uint8_t i = 0; i < FRACTION_COUNT; ++i) {
        fractionAngles[i] = static_cast<uint8_t>(min(settings.angles[i], static_cast<uint16_t>(180)));
    }
}

void initFractionator(const FractionatorSettings& settings) {
    configureFractionator(settings);
    ...
    fractionatorServo.write(fractionAngles[static_cast<uint8_t>(Fraction::HEADS)]);
    currentFraction = Fraction::HEADS;
    currentAngle = fractionAngles[static_cast<uint8_t>(Fraction::HEADS)];
...
void setBody(bool open) {
    setValveState(ValveId::BODY, open, true);
}
...
void setTails(bool open) {
    setValveState(ValveId::TAILS, open, true);
}
...
bool hasHeadsValve() {
    return isDiscreteValvePinAvailable(PIN_VALVE_HEADS);
}

bool hasBodyValve() {
    return isDiscreteValvePinAvailable(PIN_VALVE_BODY);
}

bool hasTailsValve() {
    return isDiscreteValvePinAvailable(PIN_VALVE_TAILS);
}

bool hasUnoValve() {
    return isDiscreteValvePinAvailable(PIN_VALVE_UNO);
}

bool isProductValveAvailable(RectTakeoffFraction fraction) {
    switch (fraction) {
    case RectTakeoffFraction::HEADS:
        return hasHeadsValve();
    case RectTakeoffFraction::BODY:
        return hasBodyValve();
    case RectTakeoffFraction::TAILS:
        return hasTailsValve();
...
bool isAnyTakeoffValveOpen() {
    return valveHeads || valveBody || valveTails || valveUno;
}

void closeProductValves() {
    setValveState(ValveId::HEADS, false, true);
    setValveState(ValveId::BODY, false, true);
    setValveState(ValveId::TAILS, false, true);
}

void setProductValve(RectTakeoffFraction fraction, bool open) {
    if (!open || fraction == RectTakeoffFraction::NONE) {
        closeProductValves();
        return;
    }
    ...
    switch (fraction) {
    case RectTakeoffFraction::HEADS:
        setValveState(ValveId::HEADS, true, true);
        break;
    case RectTakeoffFraction::BODY:
        if (hasBodyValve()) {
            setValveState(ValveId::BODY, true, true);
        }
        break;
    case RectTakeoffFraction::TAILS:
        if (hasTailsValve()) {
            setValveState(ValveId::TAILS, true, true);
        }
        break;
    case RectTakeoffFraction::NONE:
    default:
...
void closeAll() {
    ...
    clearValvePulse(ValveId::BODY);
    clearValvePulse(ValveId::TAILS);
    clearValvePulse(ValveId::UNO);
    setValveState(ValveId::WATER, false, false);
    setValveState(ValveId::HEADS, false, false);
    setValveState(ValveId::BODY, false, false);
    setValveState(ValveId::TAILS, false, false);
...
void update() {
    for (size_t index = 0;
         index < (sizeof(g_valvePulses) / sizeof(g_valvePulses[0]));
         ++index) {
