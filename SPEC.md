...
**Реализовано в v1.2:**
- MQTT & Home Assistant Integration с автообнаружением
- HTTP Basic Authentication
- Rate Limiting (защита от DDoS)
- PZEM-004T мониторинг энергии
- Улучшенная калибровка насоса с выбором скоростей
- Telegram Bot
- WatchDog Timer
- mDNS

**Реализовано в v1.3:**
- 📈 **История процессов** (v1.3.0)
  - Сохранение данных в SPIFFS (JSON формат)
  - Автоматическая ротация (до 50 процессов)
  - Список всех процессов в веб-интерфейсе
  - Детальный просмотр с интерактивными графиками (ApexCharts)

- 📊 **Сравнение процессов** (v1.3.1)
  - Выбор 2-5 процессов для сравнения
  - Наложенные графики температур и мощности
  - Сравнительная таблица метрик
  - Цветовая кодировка процессов

- 📤 **Экспорт данных** (v1.3.0)
  - CSV export для каждого процесса
  - JSON export для каждого процесса
  - Экспорт всей истории в JSON
  - Автоматическая загрузка через браузер

- 🔔 **Push-уведомления** (v1.3.2)
  - MQTT уведомления через Home Assistant
  - Уведомления о смене фаз процесса
  - Уведомления о завершении/остановке
  - Критические алерты безопасности
  - Sensor "Last Notification" в HA Discovery
...
### v1.4 - Профили и рецепты 📝

**Цель:** Повторяемость и автоматизация процессов

**Основные функции:**
- 🎯 **Профили процессов**
  - Сохранение всех настроек процесса под именем
  - Категории: Ректификация, Дистилляция, Затирка
  - Быстрая загрузка профиля одной кнопкой
  - Импорт/экспорт профилей (JSON)

- 🔄 **Повторяемые рецепты**
  - Библиотека рецептов с параметрами:
    - Температурные режимы
    - Объемы фракций
    - Скорости отбора
    - Мощность нагрева
  - Шаблоны для популярных процессов:
    - "Сахарная брага 40%"
    - "Зерновая брага 12%"
    - "Фруктовая дистилляция"
  - Пользовательские рецепты

- 📊 **Статистика по профилям**
  - Количество запусков
  - Средние показатели (время, выход продукта)
  - Рейтинг успешности

**Интерфейс:**
```
Web UI: Новая вкладка "📚 Профили"
- Список сохраненных профилей
- Кнопки: Создать, Загрузить, Редактировать, Удалить
- Превью параметров профиля
```
