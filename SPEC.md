...
### v1.3 - Расширенная аналитика и история 📊

**Цель:** Сохранение и анализ данных процессов

**Основные функции:**
- 📈 **История процессов**
  - Сохранение данных завершенных перегонов в SPIFFS/SD
  - Структура: дата, режим, параметры, метрики (время, температуры, объемы)
  - Список последних 20 процессов в Web UI
  - Детальный просмотр каждого процесса

- 📊 **Сравнение процессов**
  - Графики наложением (2-3 процесса)
  - Сравнение ключевых метрик в таблице
  - Поиск оптимальных параметров

- 📉 **Экспорт данных**
  - CSV export (совместимо с Excel)
  - JSON export для обработки
  - Кнопка "Скачать отчет" по каждому процессу

- 📱 **Push-уведомления**
  - Интеграция с Home Assistant Companion App
  - Уведомления о смене фаз
  - Уведомления о завершении/ошибках
  - Критические алерты (безопасность)

**API endpoints:**
```
GET  /api/history              - Список процессов
GET  /api/history/{id}         - Данные процесса
GET  /api/history/{id}/export  - CSV export
POST /api/history/{id}/compare - Сравнение
DEL  /api/history/{id}         - Удалить процесс
```

**Хранилище:**
- `/data/history/process_{timestamp}.json`
- Ротация: max 50 файлов или 2 МБ
