...
Импорт всегда создаёт новый пользовательский профиль с уникальным локальным ID, сбрасывает runtime usage statistics и сохраняет `validation` baseline из snapshot для последующей проверки совместимости. Safety-параметры профиля не применяются автоматически: перед запуском их проверяет process preflight.
