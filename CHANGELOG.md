...
### Исправлено
- **BUG: HTTP 400 при сохранении калибровки помпы** — Фронтенд отправлял `{knownVolume, elapsedMs, targetSpeed}` на эндпоинт `/api/calibration/pump`, который ожидал только `{mlPerRev}` или `{knownVolume, steps}`. Оба фронтенда (`src/web/settings/calibration.js` и `data/calibration.html`) переведены на правильный трёхшаговый API: `calibrate/start` при запуске насоса → `calibrate/stop` при остановке → `calibrate/finish` при сохранении результата. Теперь для расчёта `mlPerRevolution` используется реальный счётчик шагов, а не приближение через время и скорость. (antigravity)
- Исправлено замирание счетчика шагов при запросе `getTotalSteps()` за счет прямого чтения из драйвера. (gemini)
- Починена боковая панель навигации в веб-интерфейсе (добавлены недостающие классы `.tab` для элементов меню). (gemini)
- Добавлена вкладка "Оборудование" (Equipment) в боковую панель для управления периферией. (gemini)
- Удален дублирующийся пункт "Графики" из основного меню. (gemini)
- Добавлена регистрация заголовка для страницы профилей в `tabs.js`. (gemini)
