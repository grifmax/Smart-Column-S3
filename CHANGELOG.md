...
### Изменено
- В `src/interface/telegram.cpp` удалены кастомные polling-обходы, отдельные transport-эксперименты и фоновые Telegram worker-задачи; модуль возвращён к простой library-driven схеме на `FastBot2` с deferred init, базовыми командами и редким `tick()` по таймеру. (codex)
- В `src/interface/telegram.cpp` логика бота упрощена до минимального набора `/status`, `/health`, `/stop`, `/help` и inline-кнопок без самодельного HTTPS `getUpdates`, чтобы восстановить предсказуемое поведение на той же библиотеке, которая у тебя работает в других проектах. (codex)
- Версия прошивки поднята до `2.0.55` как отдельный rollback-fix после неудачных Telegram hotfix-экспериментов, которые только усложнили модуль и увели его от нормального пути `FastBot2`. (codex)
