...
### Changed
- Compact-кнопка `SAFETY: OK/ТРЕВОГА` на idle Home открывает тот же live `SAFETY_VIEW`, что и context menu процесса; Safety стал доступен во всех состояниях без добавления второго safety-контура. (codex)
...
### Changed
- TFT `PREFLIGHT` теперь показывает полный backend-checklist постранично: до 24 пунктов с исходными `title`, `good/warn/danger` и точным `detail`; отображение больше не ограничено первыми четырьмя проверками. (codex)
...
### Added
- В TFT-подготовке режима появился `PROFILE_PICKER`: до 100 сохранённых совместимых профилей с четырьмя строками на страницу, отметкой текущего выбора и preview effective settings. Выбор остаётся в `setupDraft`, не пишет NVS при навигации и применяется только при явном запуске. (codex)

### Changed
- Backend pre-flight принимает внутренний `params.profileId` override для проверки выбранного TFT-черновика; после фактического применения профиля TFT повторяет ту же проверку до вызова FSM. (codex)
...
### Changed
- TFT получил единый защищённый запуск: `MODE_PICKER → MODE_SETUP → PREFLIGHT`. Выбор одного из семи поддерживаемых режимов больше не запускает FSM; экран подготовки показывает активный профиль, краткие параметры и обязательные датчики, а `Старт` повторно использует тот же backend `buildProcessPreflight()`, что и Web UI. (codex)
- `CONTROL` оставлен только для управления уже активным процессом; прежний прямой запуск из него удалён. (codex)
...
### Fixed
- Длинное OTA-обновление LittleFS больше не прерывается task watchdog: основной цикл теперь обслуживает watchdog, пока `ArduinoOTA` принимает файловую систему. (codex)
...
### Fixed
- Локализованы фазы процесса и операторские подсказки: пользовательский интерфейс больше не показывает технические токены наподобие `heating`, а сообщения автоматики и safety-контуров переведены на русский язык. (codex)
- Исправлена кодировка сообщений совместимости профилей. (codex)
...
### Fixed
- Усилен импорт профилей для community snapshots: batch-импорт больше не перезаписывает профили при совпадении секунды, а validation baseline сохраняется для последующей проверки совместимости; удалённый каталог сообщества остаётся следующим cloud-этапом. (codex)
...
### Fixed
- Добавлена явная версия схемы NVS и миграция старых установок: новые опциональные контуры получают безопасные defaults и сохраняются после первого запуска новой прошивки. (codex)
...
### Fixed
- Добавлен safety proof guard для сервисных тестов: физические воздействия блокируются при активном процессе, аварии, небезопасном состоянии и demo mode; добавлены host-side проверки матрицы защит. (codex)
## [2.4.82] - 2026-07-17

### Fixed
- Завершена поддержка lite-комплектаций датчиков: health-check и счётчики ошибок учитывают только реально установленные роли, а API и Web UI показывают tier `lite/full` и доступные режимы. (codex)
## [2.4.83] - 2026-07-17

### Added
- Заложена опциональная поддержка независимого аварийного реле нагрева: отдельный compile-time GPIO, полярность, safe-off при старте и emergency stop, а также live-диагностика канала. По умолчанию реле отключено до выбора физического пина. (codex)
## [2.4.84] - 2026-07-17

### Added
- Опциональный safety-контур датчиков протечки и паров: leak-сенсор ADS1115 A3 читается в runtime и переводит систему в безопасную остановку при срабатывании или потере связи; GPIO vapor-каналы заложены compile-time флагом и по умолчанию отключены до выбора безопасных пинов. (codex)
